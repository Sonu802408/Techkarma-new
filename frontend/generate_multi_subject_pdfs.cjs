...
// Find all targeted markdown files (e.g. class_10_math_ch1_notes.md, class_10_socialstudies_ch1_notes.md)
const files = fs.readdirSync(brainDir).filter(f =>
    f.startsWith('class_10_') &&
    f.endsWith('_notes.md') &&
    !f.includes('science') // we already did science
);

async function generateBatch() {
    console.log(`Found ${files.length} markdown file(s) to process for multi-subject.`);
    const browser = await puppeteer.launch({
        args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage']
    });

    for (const file of files) {
        // Extract subject and chapter number from filename (e.g., class_10_math_ch1_notes.md)
        const match = file.match(/class_10_(.*)_ch(\d+)_notes\.md/);
        if (!match) continue;
        const subject = match[1]; // e.g., math, socialstudies, english
        const chNum = match[2];

        const mdPath = path.join(brainDir, file);
        const pdfPath = path.join(pdfDir, `class10-english-${subject}-ch${chNum}.pdf`);

        console.log(`Processing ${subject.toUpperCase()} Chapter ${chNum}...`);
        ...
        <head>
            <meta charset="UTF-8">
            <title>Class 10 ${subject.toUpperCase()} Notes - Chapter ${chNum}</title>
            ...
            <style>
                ...
                .markdown-body table {
                    ...
                    box-shadow: 0 4px 8px rgba(0,0,0,0.05); border-radius: 10px; overflow: hidden;
                }
                ...
                .markdown-alert { padding: 22px; margin: 28px 0; border-radius: 12px; border-left: 8px solid; }
                .markdown-alert p { margin-bottom: 0; font-size: 1.05em; }
                .markdown-alert-title { display: flex; align-items: center; font-family: 'Poppins', sans-serif; font-size: 1.25em; font-weight: 700; margin-bottom: 12px; text-transform: uppercase; }
                .markdown-alert-title svg { margin-right: 12px; width: 24px; height: 24px; }
                .markdown-alert-important { border-color: #EF4444; background: linear-gradient(to right, #FEF2F2, white); }
                .markdown-alert-important .markdown-alert-title { color: #DC2626; }
                .markdown-alert-important svg { fill: #DC2626; }
                .markdown-alert-tip { border-color: var(--secondary); background: linear-gradient(to right, #ECFDF5, white); }
                .markdown-alert-tip .markdown-alert-title { color: #059669; }
                .markdown-alert-tip svg { fill: #059669; }
                .markdown-alert-note { border-color: #3B82F6; background: linear-gradient(to right, #EFF6FF, white); }
                .markdown-alert-note .markdown-alert-title { color: #2563EB; }
                .markdown-alert-note svg { fill: #2563EB; }
                .markdown-alert-warning { border-color: var(--accent); background: linear-gradient(to right, #FFFBEB, white); }
                .markdown-alert-warning .markdown-alert-title { color: #D97706; }
                .markdown-alert-warning svg { fill: #D97706; }
                
                .mermaid { text-align: center; margin: 35px 0; background: #F8FAFC; padding: 25px; border-radius: 12px; border: 2px dashed #CBD5E1; }
                .markdown-body code { background-color: #EEF2FF; color: #4338ca; padding: 0.25em 0.5em; border-radius: 6px; font-weight: 700; }
                .markdown-body ul, .markdown-body ol { margin-top: 10px; margin-bottom: 10px; padding-left: 2em; }
            ...
            <script>
                document.querySelectorAll('blockquote').forEach(bq => {
                    ...
                    if (match) {
                        ...
                        const titleWord = type === 'important' ? 'Important Points 📌' : type === 'warning' ? 'Common Mistakes ❌' : type === 'tip' ? 'Quick Tip 💡' : 'Note 📝';
    ...
    await browser.close();
    console.log("All multi-subject files processed!");
}
